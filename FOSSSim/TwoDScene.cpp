...
VectorXs& TwoDScene::computeF()
{
    assert(m_f.size() == m_m.size());
    int size = m_f.size();
...
void TwoDScene::resizeSystem( int num_particles )
{
    ...
    m_f.resize(2*num_particles); // for my own stuff
...
void TwoDScene::insertForce( Force* newforce )
{
    ...
    computedF = false; // F must be recomputed
}

scalar TwoDScene::computeKineticEnergy() const
{
    scalar T = 0.0;
    int num_particles = getNumParticles(); // cache this number for faster looping
    for( int x = 0, y = 1; x < num_particles; x += 2, y += 2) {
        // T = 1/2 * m * (v . v)
        T += 0.5 * m_m[x] * ((m_v[x] * m_v[x]) + (m_v[y] * m_v[y]));
    }
    return T;  
}
...
void TwoDScene::checkConsistency()
{
    ...
    assert( (m_f.array()==m_f.array()).all() );
